...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # (layers_dims[i], layers_dims[i+1]) corresponds to the Wi, bi of layer i
        layers_dims = np.hstack([input_dim, hidden_dims, num_classes])

        for layer_idx in range(self.num_layers):
          self.params['W'+str(layer_idx+1)] = weight_scale * np.random.randn(layers_dims[layer_idx], layers_dims[layer_idx+1])
          self.params['b'+str(layer_idx+1)] = np.zeros(layers_dims[layer_idx+1])
        
        # store batch/layer norm parameters
        if self.normalization != None:
          for layer_idx in range(self.num_layers-1):
            self.params['gamma'+str(layer_idx+1)] = np.ones(layers_dims[layer_idx+1])
            self.params['beta'+str(layer_idx+1)] = np.zeros(layers_dims[layer_idx+1])
    ...
    def loss(self, X, y=None):
        ...
        cur_x = X # the former layer output, operating on current layer
        cache = []
        ...
        for layer in range(self.num_layers-1):
          ...
          cur_x, relu_cache = affine_norm_relu_forward(cur_x, w, b, gamma, beta, bn_params, self.normalization, 
                                                      self.use_dropout, self.dropout_param)
          cache.append(relu_cache)
        
        w = self.params['W'+str(self.num_layers)]
        b = self.params['b'+str(self.num_layers)]
        scores, final_cache = affine_forward(cur_x, w, b)
        ...
        for layer in range(self.num_layers):
          loss += 0.5 * np.sum(self.params['W'+str(layer+1)] * self.params['W'+str(layer+1)]) * self.reg

        # gradient calculation
        dout = softgrad
        dxn, dwn, dbn = affine_backward(dout, cache[self.num_layers-1])
        grads['W'+str(self.num_layers)] = dwn + self.reg * self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = dbn
        dout = dxn
        for layer in range(self.num_layers-2, -1, -1):
          dxn, dwn, dbn, dgamma, dbeta = affine_norm_relu_backward(dout, cache[layer], self.normalization, self.use_dropout)
          
          if self.normalization != None:
            grads['gamma'+str(layer+1)] = dgamma
            grads['beta'+str(layer+1)] = dbeta

          grads['W'+str(layer+1)] = dwn + self.reg * self.params['W'+str(layer+1)]
          grads['b'+str(layer+1)] = dbn
          dout = dxn
